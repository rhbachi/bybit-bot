...
from strategy_scalping_3candles import (
    apply_indicators_5m,
    detect_trend_5m,
...
def place_sl_tp_orders(symbol, side, qty, entry_price, sl_price, tp_price):
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Scalping - SL/TP placés: SL={round(sl_price, 2)} | TP={round(tp_price, 2)}", flush=True)
            return True
        except Exception as e1:
            print(f"⚠️ Scalping - Méthode 1 échouée: {e1}", flush=True)
            
            # Méthode 2 : Ordres conditionnels
            ...
            print(f"✅ Scalping - SL/TP placés (méthode 2)", flush=True)
            return True
    except Exception as e:
...
def run():
    ...
    print("🤖 Bot SCALPING 3 Bougies V1.0 démarré", flush=True)
    send_telegram("🤖 Bot SCALPING 3 Bougies démarré\n📊 5m trend + 1m execution\n⚡ SL: 0.25% | TP: 2%")
